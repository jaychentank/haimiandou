    ...
    def get_target_tensor(self, input, target_is_real):
        ...
        if target_is_real:
            create_label = ((self.real_label_var is None) or
                            (self.real_label_var.numel() != input.numel()))  # numel查看一个张量有多少个元素
            if create_label:
                real_tensor = self.Tensor(input.size()).fill_(self.real_label)
                self.real_label_var = Variable(real_tensor, requires_grad=False)
                # Variable自动微分变量，用于构建计算图
                # tensor：要封装的张量 requires_grad：bool，是否要求计算导数
...
class UnetGenerator3d(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
        ...
        # construct unet structure
        unet_block = UnetSkipConnectionBlock3d(ngf * 8, ngf * 16, norm_layer=norm_layer, innermost=True)
        for i in range(num_downs - 5):
            unet_block = UnetSkipConnectionBlock3d(ngf * 8, ngf * 16, unet_block, norm_layer=norm_layer,
                                                   use_dropout=use_dropout)
